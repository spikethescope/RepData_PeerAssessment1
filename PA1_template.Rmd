...
library(dplyr)
library(ggplot2)
setwd("D:\\Google Drive\\Courseera materials\\Reproducible Research\\Week 1 Project")
...
the graph. Do a descriptive of this
...
First create a time series plot of average number of steps taken
```{r echo=TRUE}
plot(ts(proc_data[["avg_steps_per_day"]]), col = "red", xlab = "Time series plot", ylab = "Total steps per day")
axis(2,labels = T)
```
Find average number of steps for each 5 minute interval
Need to group by interval for this and then calculate avg. steps.
using dplyr for this and creating a new variable o
```{r echo=TRUE}
...
Comparing with missing data descriptive, imputed data mean value remains same. Only third quartile value has changed.



## Are there differences in activity patterns between weekdays and weekends?
creating a days of week factor in the following process.
```{r echo=TRUE}
days_of_week <- weekdays(NAfilled$date, abbreviate = T)
```

create a factor variable for weekday, weekend using the above variable.
